...
func main() {
	...
	//chdorizontalProduct(arr)
	leftDiagonalProduct(arr)
}

func leftDiagonalProduct(arr [20][20]int) {
	var i, j, product int
	product = 0
	...
	for k := 0; k < 20; k++ {
		...
		for {
			...
			for i = 0; i < 20; i++ {
				...
				temp = temp * arr[i][j] * arr[i+1][j-1] * arr[i+2][j-2] * arr[i+3][j-3]
...
func verticalProduct(arr [20][20]int) {
	var i, j int
	var a, b, c, d, sum, temp int
...
func horizontalProduct(arr [20][20]int) {
	var a, b, c, d, sum, temp int
